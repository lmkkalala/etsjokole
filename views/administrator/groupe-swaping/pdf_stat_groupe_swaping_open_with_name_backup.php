...
include("../../../models/pdf-generator/pdfclass_landscape.php");
...
$pdf->Cell(280, 10, utf8_decode("Income Financial Report"), 0, 0, 'C');
$pdf->Ln(10);
$pdf->SetFont('Arial', 'B', 12);
$pdf->Cell(280, 10, utf8_decode($titre_document), 0, 0, 'C');
$pdf->Ln(15);
$n = 0;

$pdf->SetFont('Times', 'B', 12);
$pdf->SetWidths(array(8, 15, 25, 25, 60, 60, 60, 10, 10, 10, 10));
//foreach ($theads as $thead) {
$pdf->Row(array(decode('No.'), decode('Category'), decode('Department'), decode('Restaurant'), decode("For breakfast"), decode("For lunch"), decode("For dinner"), decode('Breakfast'), decode('Lunch'), decode('Dinner'), decode("Total")));
            ...
            if ($fonction['active']) {
                ...
                foreach ($services as $service) {
                    if ($service['active']) {
                        ...
                        $array_recette_par_typerepas = [];
                        array_push($array_recette_par_typerepas, [$cumul_breakfast, $nombre_personne_breakfast, $array_info_swaping_breakfast]);
                        array_push($array_recette_par_typerepas, [$cumul_lunch, $nombre_personne_lunch, $array_info_swaping_lunch]);
                        array_push($array_recette_par_typerepas, [$cumul_dinner, $nombre_personne_dinner, $array_info_swaping_dinner]);
...
foreach ($array_recette_categorie as $categories) {
    $pdf->SetFont('Times', 'B', 12);
    $pdf->Row(array(decode($n_categorie + 1), decode($array_groupeswaping[$n_categorie]), decode(" "), decode(" "), decode(" "), decode(" "), decode(""), decode(" "), decode(" "), decode(" "), decode(" ")));
    $pdf->SetFont('Times', '', 12);
    $n_categorie++;
    $n_departement = 0;
    $cumul_departement = 0;
    ...
    foreach ($categories as $departements) {
        $pdf->SetFont('Times', 'B', 12);
        $pdf->Row(array(decode(" "), decode(" "), decode($array_fonction[$n_departement]), decode(" "), decode(" "), decode(" "), decode(""), decode(" "), decode(" "), decode(" "), decode(" ")));
        $pdf->SetFont('Times', '', 12);
        $n_departement++;
        $n_restaurant = 0;
        $cumul_v1 = 0;
        ...
        foreach ($departements as $restaurants) {
            ...
            $pdf->Row(array(decode(" "), decode(" "), decode(" "), decode($array_service[$n_restaurant]), decode(" "), decode(" "), decode(""), decode($v1), decode($v2), decode($v3), decode(($somme_restaurant))));
            $n_restaurant++;
            foreach ($restaurants[0][2] as $people_breakfast) {
                $pdf->SetTextColor(30, 144, 255);
                $pdf->Row(array(decode(" "), decode(" "), decode(" "), decode(" "), decode($people_breakfast), decode(" "), decode(" "), decode(" "), decode(" "), decode(" "), decode(" ")));
                $pdf->SetTextColor(0, 0, 0);
            }
            foreach ($restaurants[1][2] as $people_lunch) {
                $pdf->SetTextColor(30, 144, 255);
                $pdf->Row(array(decode(" "), decode(" "), decode(" "), decode(" "), decode(" "), decode($people_lunch), decode(" "), decode(" "), decode(" "), decode(" "), decode(" ")));
                $pdf->SetTextColor(0, 0, 0);
            }
            foreach ($restaurants[2][2] as $people_dinner) {
                $pdf->SetTextColor(30, 144, 255);
                $pdf->Row(array(decode(" "), decode(" "), decode(" "), decode(" "), decode(" "), decode(" "), decode($people_dinner), decode(" "), decode(" "), decode(" "), decode(" ")));
                $pdf->SetTextColor(0, 0, 0);
            }
        }
        ...
        if ($cumul_v1 != 0) {
            $cumul_v1 = "" . $cumul_hashtag_v1;
        }
        if ($cumul_v2 != 0) {
            $cumul_v2 = "" . $cumul_hashtag_v2;
        }
        if ($cumul_v3 != 0) {
            $cumul_v3 = "" . $cumul_hashtag_v3;
        }
        $pdf->SetTextColor(255, 165, 0);
        $pdf->Row(array(decode(" "), decode(" "), decode(" "), decode("Total : "), decode(" "), decode(" "), decode(""), decode($cumul_v1), decode($cumul_v2), decode($cumul_v3), decode(($somme_cumul_v))));
        $pdf->SetTextColor(0, 0, 0);
    }
    ...
    if ($cumul_v1_departement != 0) {
        $cumul_v1_departement = "" . $cumul_hashtag_v1_departement;
    }
    if ($cumul_v2_departement != 0) {
        $cumul_v2_departement = "" . $cumul_hashtag_v2_departement;
    }
    if ($cumul_v3_departement != 0) {
        $cumul_v3_departement = "" . $cumul_hashtag_v3_departement;
    }
    ...
    $pdf->Row(array(decode(" "), decode(" "), decode("Total : " . ($cumul_departement)), decode(" "), decode(" "), decode(" "), decode(""), decode($cumul_v1_departement), decode($cumul_v2_departement), decode($cumul_v3_departement), decode(" ")));
    $pdf->SetTextColor(0, 0, 0);
}
$cumul_categorie = ($cumul_v1_categorie + $cumul_v2_categorie + $cumul_v3_categorie);
if ($cumul_v1_categorie != 0) {
    $cumul_v1_categorie = " " . $cumul_hashtag_v1_categorie;
}
if ($cumul_v2_categorie != 0) {
    $cumul_v2_categorie = " " . $cumul_hashtag_v2_categorie;
}
if ($cumul_v3_categorie != 0) {
    $cumul_v3_categorie = " " . $cumul_hashtag_v3_categorie;
}
...
$pdf->Row(array(decode(" "), decode("Total : " . $cumul_categorie), decode(" "), decode(" "), decode(" "), decode(" "), decode(""), decode($cumul_v1_categorie), decode($cumul_v2_categorie), decode($cumul_v3_categorie), decode(" ")));
$pdf->SetTextColor(0, 0, 0);
