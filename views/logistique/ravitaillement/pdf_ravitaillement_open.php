...
$n = 0;
$pdf->SetFont('Times', 'B', 11);
$pdf->SetWidths(array(8, 30, 50, 10, 20, 15, 15, 15, 25));
//foreach ($theads as $thead) {
$pdf->Row(array(decode('N°'), decode('Date'), decode('Order'), decode('Qty'), decode('Cost delivery'), decode('Actual value'), decode('Unit price'), decode('Receipt value'), decode('Exp. Date')));
...
foreach ($fournisseurs as $fournisseur) {
    ...
    foreach ($ravitaillements as $ravitaillement) {
        if ($ravitaillement['fournisseur_id'] == $fournisseur['id']) {
            ...
            $pdf->Row(array(decode($v1), decode($v2), decode($v3), decode($v4), decode($v5), decode($v6), decode($v7), decode($v8), decode($v10)));
    ...
    $pdf->Row(array(decode(""), decode("Total : " . $cumul_total_fournisseur . " USD"), decode(""), decode(""), decode(""), decode(""), decode(""), decode(""), decode("")));
    $pdf->SetTextColor(0, 0, 0);
}
