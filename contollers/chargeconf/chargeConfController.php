...
if (isset($_POST['bt_enregistrer'])) {
    ...
    $etatcivil = $_POST['cb_etatcivil'];
    $nombreenfant = $_POST['tb_nombreenfant'];
    $nombrefemme = $_POST['tb_nombrefemme'];
    $employeId = $_POST['cb_employe'];
    if ($dateconf != "" && $etatcivil != "none" && $nombreenfant != "" && $nombrefemme != "" && $employeId != 0) {
        if (1) {
            $bdchargeconf = new BdChargeConf();
            if (1) {
                if ($bdchargeconf->desactiveChargeConfByEmploye($employeId)) {
                    if ($bdchargeconf->addChargeConf($dateconf, $etatcivil, $nombreenfant, $nombrefemme, $employeId)) {
                        $reponse = "succes";
                    } else {
    ...
    header('Location:../../views/home.php?link=' . sha1("admin_chargeconf_add") . '&link_up=' . sha1("home_admin_chargeconf") . '&reponse=' . sha1($reponse));
    die;
}
