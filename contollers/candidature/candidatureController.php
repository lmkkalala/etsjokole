...
if (isset($_POST['bt_enregistrer'])) {
    $datesoumission = $_POST['tb_datesoumission'];
    $etatAcceptation = 0;
    $offreemploieId = $_POST['cb_offreemploie'];
    $candidatId = $_POST['cb_candidat'];
    if ($datesoumission != "" && $offreemploieId != 0 && $candidatId != 0) {
        if (1) {
            $bdcandidature = new BdCandidature();
            $candidatures = $bdcandidature->getCandidatureByOffreEmploieByCandidat($offreemploieId, $candidatId);
            $i = 0;
            foreach ($candidatures as $value) {
                $i++;
            }
            if ($i == 0) {
                if ($bdcandidature->addCandidature($datesoumission, $etatAcceptation, $offreemploieId, $candidatId)) {
                    $reponse = "succes";
                } else {
    ...
    header('Location:../../views/home.php?link=' . sha1("admin_candidature_add") . '&link_up=' . sha1("home_admin_candidat") . '&reponse=' . sha1($reponse));die;
}
...
if (isset($_POST['bt_selection'])) {
    ...
    header('Location:../../views/home.php?link=' . sha1("admin_candidature_accepte_candidature_all") . '&reponse=' . sha1($reponse) . '&link_up=' . sha1("home_admin_candidat"));
}
?>