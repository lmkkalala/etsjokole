...
class BdChargeConf {
    ...
    function addChargeConf($dateconf,$etatcivil,$nombreenfant,$nombrefemme,$employeId) {
        try {
            $bd = ConnexionM::connecter();
            $query = $bd->prepare("INSERT INTO chargeconf(dateConf,etatCivil,nombreEnfant,nombreFemme,employeId) VALUES(?,?,?,?,?)");
            $query->execute([$dateconf,$etatcivil,$nombreenfant,$nombrefemme,$employeId]);
            $query->closeCursor();
            return TRUE;
        } catch (Exception $ex) {
            return FALSE;
    ...
    function desactiveChargeConfByEmploye($idemploye) {
        try {
            $bd = ConnexionM::connecter();
            $query = $bd->prepare("UPDATE chargeconf SET active=? WHERE employeId=?");
    ...
    function getChargeConfAllDesc() {
        $bd = ConnexionM::connecter();
        $reponse = $bd->query('SELECT CC.id AS ccId,CC.dateConf,CC.etatCivil,CC.nombreEnfant,CC.nombreFemme,CC.active AS ccActive,E.matricule,E.dateRecrutement,CR.id,CR.dateSoumission,CR.etatAcceptation,CR.active,CR.etatAcceptation,C.nom,C.postnom,C.prenom,C.sexe,O.numero,O.libelle,O.dateLancement,O.dateCloture FROM chargeconf CC INNER JOIN (employe E INNER JOIN (candidature CR INNER JOIN candidat C ON(CR.candidatId=C.id) INNER JOIN offreemploie O ON(CR.offreEmploieId=O.id)) ON(E.candidatureId=CR.id)) ON(CC.employeId=E.id) ORDER BY CC.id DESC');
        return $reponse->fetchAll();
        $reponse->closeCursor();
    }
    
    function getChargeConfByEmploye($idemploye) {
        $bd = ConnexionM::connecter();
        $reponse = $bd->query("SELECT CC.id AS ccId,CC.dateConf,CC.etatCivil,CC.nombreEnfant,CC.nombreFemme,CC.active AS ccActive,E.matricule,E.dateRecrutement,CR.id,CR.dateSoumission,CR.etatAcceptation,CR.active,CR.etatAcceptation,C.nom,C.postnom,C.prenom,C.sexe,O.numero,O.libelle,O.dateLancement,O.dateCloture FROM chargeconf CC INNER JOIN (employe E INNER JOIN (candidature CR INNER JOIN candidat C ON(CR.candidatId=C.id) INNER JOIN offreemploie O ON(CR.offreEmploieId=O.id)) ON(E.candidatureId=CR.id)) ON(CC.employeId=E.id) WHERE E.id='{$idemploye}' ORDER BY CC.id DESC");
        return $reponse->fetchAll();
        $reponse->closeCursor();
    }
    
    function getChargeConfById($id) {
        $bd = ConnexionM::connecter();
        $reponse = $bd->query("SELECT CC.id AS ccId,CC.dateConf,CC.etatCivil,CC.nombreEnfant,CC.nombreFemme,CC.active AS ccActive,E.matricule,E.dateRecrutement,CR.id,CR.dateSoumission,CR.etatAcceptation,CR.active,CR.etatAcceptation,C.nom,C.postnom,C.prenom,C.sexe,O.numero,O.libelle,O.dateLancement,O.dateCloture FROM chargeconf CC INNER JOIN (employe E INNER JOIN (candidature CR INNER JOIN candidat C ON(CR.candidatId=C.id) INNER JOIN offreemploie O ON(CR.offreEmploieId=O.id)) ON(E.candidatureId=CR.id)) ON(CC.employeId=E.id) WHERE CC.id='{$id}' ORDER BY CC.id DESC");
        return $reponse->fetchAll();
        $reponse->closeCursor();
    }
