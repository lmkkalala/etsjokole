...
class BdVentePOS
{
    ...
    public function addVentePOS($id,$serviceId)
    {
        try {
            $bd = Connexion::connecter();
            $query = $bd->prepare('INSERT INTO ventepos(id,dateEnreg,heure,serviceId) VALUES(?,?,?,?)');
            $query->execute([$id,date('Y-m-d'), date('H:i:s'), $serviceId]);
            $query->closeCursor();
    ...
    public function getVentePOSRecentId()
    {
        try {
            $bd = Connexion::connecter();
            $query = $bd->query('SELECT MAX(id) as recentId FROM ventepos');
            $data=$query->fetchAll();
            $query->closeCursor();
    ...
    public function getVentePOSAll()
    {
        try {
            $bd = Connexion::connecter();
            $query = $bd->query('SELECT * FROM ventepos');
            $data=$query->fetchAll();
            $query->closeCursor();
