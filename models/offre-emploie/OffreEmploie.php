...
class BdOffreEmploie {

    public function __construct() {
        
    }

    function addOffreEmploie($datelancement,$datecloture,$libelle,$numero) {
        try {
            $bd = ConnexionM::connecter();
            $query = $bd->prepare("INSERT INTO offreemploie(dateLancement,dateCloture,libelle,numero) VALUES(?,?,?,?)");
            $query->execute([$datelancement,$datecloture,$libelle,$numero]);
            $query->closeCursor();
            return TRUE;
        } catch (Exception $ex) {
            return FALSE;
    ...
    function getOffreEmploieAllDesc() {
        $bd = ConnexionM::connecter();
        $reponse = $bd->query('SELECT * FROM offreemploie ORDER BY id DESC');
        return $reponse->fetchAll();
        $reponse->closeCursor();
    }
