...
class BdComposanteSalaire {

    public function __construct() {
        
    }

    function addComposanteSalaire($code,$designation,$unite,$type,$nature,$defaultquantite) {
        try {
            $bd = ConnexionM::connecter();
            $query = $bd->prepare("INSERT INTO composantesalaire(code,designation,unite,type,nature,defaultQuantite) VALUES(?,?,?,?,?,?)");
            $query->execute([$code,$designation,$unite,$type,$nature,$defaultquantite]);
            $query->closeCursor();
            return TRUE;
        } catch (Exception $ex) {
            return FALSE;
    ...
    function getComposanteSalaireAllDesc() {
        $bd = ConnexionM::connecter();
        $reponse = $bd->query('SELECT * FROM composantesalaire ORDER BY id DESC');
        return $reponse->fetchAll();
        $reponse->closeCursor();
    }
