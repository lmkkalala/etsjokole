...
class BdLigneImposition {

    public function __construct() {
        
    }

    function addLigneImposition($idconfimposition,$idbulletin) {
        try {
            $bd = ConnexionM::connecter();
            $query = $bd->prepare("INSERT INTO lignebulletinimposition(quantite,compImpotConfId,bulletinPaieId) VALUES(?,?,?)");
            $query->execute([1,$idconfimposition,$idbulletin]);
            $query->closeCursor();
            return TRUE;
        } catch (Exception $ex) {
            return FALSE;
