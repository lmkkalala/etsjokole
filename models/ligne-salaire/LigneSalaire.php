...
class BdLigneSalaire {

    public function __construct() {
        
    }

    function addLigneSalaire($quantite,$idconfsalaire,$idbulletin) {
        try {
            $bd = ConnexionM::connecter();
            $query = $bd->prepare("INSERT INTO lignebulletinsalaire(quantite,compSalaireConfId,bulletinPaieId) VALUES(?,?,?)");
            $query->execute([$quantite,$idconfsalaire,$idbulletin]);
            $query->closeCursor();
            return TRUE;
        } catch (Exception $ex) {
            return FALSE;
