...
class BdLineSale
{
    ...
    public function addLineSale($quantite, $prix, $tauxTVA, $saleId, $productionId)
    {
        try {
            $bd = Connexion::connecter();
            $query = $bd->prepare('INSERT INTO linesale(quantite,prix,tauxTVA,saleId,productionId) VALUES(?,?,?,?,?)');
            $query->execute([$quantite, $prix, $tauxTVA, $saleId, $productionId]);
            $query->closeCursor();
    ...
    public function deleteLineSale($lineSaleId)
    {
        try {
            $bd = Connexion::connecter();
            $query = $bd->prepare('DELETE FROM linesale WHERE id=?');
            $query->execute([$lineSaleId]);
            $query->closeCursor();
    ...
    public function getlineSaleAll()
    {
        $bd = Connexion::connecter();
        $reponse = $bd->query('SELECT * FROM linesale ORDER BY id DESC');

        return $reponse->fetchAll();
        $reponse->closeCursor();
    }

    public function getlineSaleBySale($saleId)
    {
        $bd = Connexion::connecter();
        $reponse = $bd->query("SELECT * FROM linesale WHERE saleId='{$saleId}' ORDER BY id DESC");

        return $reponse->fetchAll();
        $reponse->closeCursor();
    }

    public function getlineSaleByProductionId($productionId)
    {
        $bd = Connexion::connecter();
        $reponse = $bd->query("SELECT * FROM linesale WHERE productionId='{$productionId}' ORDER BY id DESC");
