...
class BdComposanteImposition {

    public function __construct() {
        
    }

    function addComposanteImposition($designation,$unite,$type) {
        try {
            $bd = ConnexionM::connecter();
            $query = $bd->prepare("INSERT INTO composanteimposition(designation,unite,type) VALUES(?,?,?)");
            $query->execute([$designation,$unite,$type]);
            $query->closeCursor();
            return TRUE;
        } catch (Exception $ex) {
            return FALSE;
    ...
    function getComposanteImpositionAllDesc() {
        $bd = ConnexionM::connecter();
        $reponse = $bd->query('SELECT * FROM composanteimposition ORDER BY id DESC');
        return $reponse->fetchAll();
        $reponse->closeCursor();
    }
