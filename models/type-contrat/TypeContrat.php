...
class BdTypeContrat {
    ...
    function addTypeContrat($designation) {
        try {
            $bd = ConnexionM::connecter();
            $query = $bd->prepare("INSERT INTO typecontrat(designation) VALUES(?)");
    ...
    function getTypeContratAllDesc() {
        $bd = ConnexionM::connecter();
        $reponse = $bd->query('SELECT * FROM typecontrat ORDER BY id DESC');
        return $reponse->fetchAll();
        $reponse->closeCursor();
    }
