...
class BdDotation {
    ...
    function addDotation($annee, $montant,$amortissementId) {
        try {
            $bd = Connexion::connecter();
            $query = $bd->prepare("INSERT INTO dotation(dateSet,annee,montant,amortissementId) VALUES(?,?,?,?)");
            $query->execute([date('Y-m-d'),$annee,$montant,$amortissementId]);
            $query->closeCursor();
            return TRUE;
        } catch (Exception $ex) {
            return false;
    ...
    function deleteDotation($amortissementId,$annee) {
        try {
            $bd = Connexion::connecter();
            $query = $bd->prepare("DELETE FROM dotation WHERE amortissementId=? AND annee=?");
            $query->execute([$amortissementId,$annee]);
            $query->closeCursor();
            return TRUE;
        } catch (Exception $ex) {
            return false;
    ...
    function getDotationByAmortissementId($amortissementId) {
        $bd = Connexion::connecter();
        $reponse = $bd->query("SELECT D.* FROM dotation D WHERE D.amortissementId='{$amortissementId}'");
        return $reponse->fetchAll();
        $reponse->closeCursor();
    }

    function getDotationByAmortissementIdByAnnee($amortissementId,$annee) {
        $bd = Connexion::connecter();
        $reponse = $bd->query("SELECT D.* FROM dotation D WHERE D.amortissementId='{$amortissementId}' AND D.annee='{$annee}'");
        return $reponse->fetchAll();
        $reponse->closeCursor();
    }
