...
class BdCandidat {
    ...
    function addCandidat($nom,$postnom,$prenom,$sexe,$nationalite,$adresse,$datenaissance,$lieunaissance,$telephone,$email) {
        try {
            $bd = ConnexionM::connecter();
            $query = $bd->prepare("INSERT INTO candidat(nom,postnom,prenom,sexe,nationalite,adresse,dateNaissance,lieuNaissance,telephone,email) VALUES(?,?,?,?,?,?,?,?,?,?)");
            $query->execute([$nom,$postnom,$prenom,$sexe,$nationalite,$adresse,$datenaissance,$lieunaissance,$telephone,$email]);
            $query->closeCursor();
            return TRUE;
        } catch (Exception $ex) {
            return FALSE;
    ...
    function getCandidatAllDesc() {
        $bd = ConnexionM::connecter();
        $reponse = $bd->query('SELECT * FROM candidat ORDER BY id DESC');
        return $reponse->fetchAll();
        $reponse->closeCursor();
    }
