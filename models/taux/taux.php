...
class BdTaux {
    ...
    function addTaux($date, $value) {
        try {
            $bd = Connexion::connecter();
            $query = $bd->prepare("INSERT INTO taux(datetaux,value) VALUES(?,?)");
            $query->execute([$date, $value]);
            $query->closeCursor();
            return TRUE;
        } catch (Exception $ex) {
            return false;
    ...
    function activeTaux($idtaux) {
        try {
            $bd = Connexion::connecter();
            $query = $bd->prepare("UPDATE taux SET active=? WHERE id=?");
            $query->execute([1, $idtaux]);
            $query->closeCursor();
            return TRUE;
        } catch (Exception $ex) {
            return false;
